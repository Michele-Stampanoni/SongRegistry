...
buyers[songs.length - 1].push(msg.sender); // reutrns length of songs array and by reducing it by 1 returns the index of the last element which is the song we jsut created
...
function isBuyer(uint songId) public view returns(bool) { //6th bullet point to check whether msg.sender is regitered buyer
address[] storage songBuyers = buyers[songId]; //creates array 'songbuyers' and getting it from storage 
bool buyer;
...
Song storage song = songs[songId]; //getting 'song' out of the 'songs' array and retriving out of storage 
require(msg.value == song.price, "Value doesnt match price"); 
song.owner.transfer(msg.value); //another way to typecast that the owner can pay as did not specify the owner is payable
}
